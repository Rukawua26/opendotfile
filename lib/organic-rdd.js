...
const OPTIONAL_CAPTURE_LENSES = new Set(["refuter"]);
const CAPTURED_LENSES = new Set([...LENS_ORDER, ...OPTIONAL_CAPTURE_LENSES]);
...
const POINTER_FILE_NAME = ".organic-rdd/receipt";
...
export function pointerPath(projectPath) {
  return join(resolve(projectPath), POINTER_FILE_NAME);
}

export function writePointer(projectPath, reviewID) {
  const path = pointerPath(projectPath);
  ensurePrivateDir(dirname(path));
  writeFileSync(path, `${reviewID}\n`, { mode: 0o600 });
  chmodSync(path, 0o600);
}

export function readPointer(projectPath) {
  const path = pointerPath(projectPath);
  if (!existsSync(path)) return null;
  try {
    const value = readFileSync(path, "utf8").split(/\r?\n/, 1)[0].trim();
    return REVIEW_ID_PATTERN.test(value) ? value : null;
  } catch {
    return null;
  }
}

export function clearPointer(projectPath) {
  const path = pointerPath(projectPath);
...
function parseNameStatusZ(output) {
  ...
  for (let i = 0; i < parts.length; i += 1) {
    ...
    let path = normalizePath(parts[i + 1] || "");
    if (status[0] === "R" || status[0] === "C") {
      i += 1;
      path = normalizePath(parts[i + 1] || "");
    }
    if (!path) continue;
    changed.push(path);
    if (status.includes("D")) deleted.push(path);
...
function inspectGit(projectPath, options = {}) {
  let rootPath;
  try {
    rootPath = realpathSync(git(["rev-parse", "--show-toplevel"], projectPath).trim());
  } catch (error) {
    const root = error.code === "ENOENT" ? findGitMarker(projectPath) : null;
    return root
  ...
  try {
    ...
    try {
      if (options.stagedOnly) {
        status = parseNameStatusZ(git(["-c", "status.relativePaths=false", "diff", "--cached", "--name-status", "-z"], projectPath));
      } else {
    ...
    return { available: true, error: "git_status_failed", root: normalizePath(rootPath), head: null, changed_files: [], untracked_files: [], deleted_files: [] };
  }
}
...
function findSddProjectMarker(projectPath) {
  let current;
  try {
    current = realpathSync(resolve(projectPath));
  } catch {
  ...
  while (true) {
    try {
      const marker = lstatSync(join(current, ".sdd-project"));
...
function resolveGitRoot(projectPath) {
  try {
    return realpathSync(git(["rev-parse", "--show-toplevel"], projectPath).trim());
  } catch (error) {
    return error.code === "ENOENT" ? findGitMarker(projectPath) : null;
  }
}

function findProjectMarkerRoot(projectPath) {
  return resolveGitRoot(projectPath) || findSddProjectMarker(projectPath);
}

function resolveProjectRoot(workspacePath) {
  const originalWorkspace = isAbsolute(workspacePath) ? workspacePath : resolve(workspacePath);
  const resolved = resolveProject(originalWorkspace);
  const gitRoot = resolveGitRoot(resolved);
  if (gitRoot) return { projectPath: gitRoot, workspacePath: originalWorkspace, canonicalWorkspacePath: resolved, gitAvailable: true, source: "git" };
  const sddRoot = findSddProjectMarker(resolved);
  if (sddRoot) return { projectPath: sddRoot, workspacePath: originalWorkspace, canonicalWorkspacePath: resolved, gitAvailable: false, source: "sdd" };
  return { projectPath: resolved, workspacePath: originalWorkspace, canonicalWorkspacePath: resolved, gitAvailable: false, source: "workspace" };
}
...
function rejectSymlinkComponents(project, lexicalPath, input) {
  ...
  if (rel === "" || rel === ".." || rel.startsWith(`..${sep}`) || isAbsolute(rel)) {
    throw new OrganicRddError("project_mismatch", `Candidate file is outside the project: ${input}`);
  }
  let current = project;
...
function inspectCandidate(projectPath, files, { legacy = false } = {}) {
  ...
  const resolved = legacy
    ? { projectPath: resolveProject(projectPath), workspacePath: projectPath, canonicalWorkspacePath: resolveProject(projectPath) }
    : resolveProjectRoot(projectPath);
  const { projectPath: project, workspacePath: workspace, canonicalWorkspacePath: workspaceBase } = resolved;
  ...
  for (const input of files) {
    ...
    const lexicalPath = isAbsolute(input) ? resolve(input) : resolve(workspaceBase, input);
    ...
    if (!legacy) {
      const nestedRoot = findProjectMarkerRoot(dirname(actualPath));
      if (nestedRoot && nestedRoot !== project) {
        throw new OrganicRddError("project_mismatch", `Candidate file belongs to a nested project: ${input}`);
      }
    }
    let fd;
    try {
      ...
      if (!pathIsInside(project, openedPath)) {
        throw new OrganicRddError("project_mismatch", `Candidate file escaped the project: ${input}`);
      }
...
function validateReceipt(receipt, reviewID) {
  ...
  if (receipt.project_id !== undefined) {
    if (!/^[a-f0-9]{64}$/.test(receipt.project_id) || receipt.project_id !== projectId(receipt.project_path)) {
      invalid("invalid project_id");
    }
    if (resolve(receipt.project_path) !== receipt.project_path) invalid("project_path is not canonical");
    if (existsSync(receipt.project_path) && realpathSync(receipt.project_path) !== receipt.project_path) {
      invalid("project_path is not canonical");
    }
    if (typeof receipt.workspace_path !== "string" || !isAbsolute(receipt.workspace_path)) invalid("invalid workspace_path");
    if (receipt.git_root !== null && (typeof receipt.git_root !== "string" || !isAbsolute(receipt.git_root))) {
      invalid("invalid git_root");
    }
    if (receipt.git_root && receipt.git_root !== receipt.project_path) invalid("git_root does not match project_path");
  }
  ...
  for (const result of receipt.captured_lenses) {
    if (!result || !CAPTURED_LENSES.has(result.lens) || captured.has(result.lens) || !LENS_STATUSES.has(result.status)) {
  ...
  if (receipt.git !== undefined) {
    ...
    if (receipt.project_id !== undefined) {
      if (receipt.git.available) {
        if (receipt.git_root !== receipt.project_path || receipt.git.root !== receipt.project_path) {
          invalid("Git identity does not match project_path");
        }
      } else if (receipt.git_root !== null || receipt.git.root !== null) {
        invalid("non-Git receipt has Git identity");
      }
    }
  }
  if (receipt.project_id !== undefined && receipt.git === undefined) invalid("missing git state");
  if (receipt.project_id !== undefined && !Array.isArray(receipt.manifest_warnings)) invalid("missing manifest warnings");
  if (receipt.manifest_warnings !== undefined) {
    ...
    if (receipt.project_id !== undefined
      && JSON.stringify(receipt.manifest_warnings) !== JSON.stringify(manifestWarnings(receipt.candidate_files, receipt.git))) {
      invalid("manifest warnings do not match Git snapshot");
    }
  }
...
export function createOrganicRdd({ storeDir } = {}) {
  ...
  function effectiveReceiptProject(receipt) {
    if (receipt.project_id) return resolveProject(receipt.project_path);
    const roots = new Set();
    for (const file of receipt.candidate_files) {
      const markerRoot = findProjectMarkerRoot(dirname(resolve(receipt.project_path, file.path)));
      if (markerRoot) roots.add(markerRoot);
    }
    return roots.size === 1 ? [...roots][0] : resolveProject(receipt.project_path);
  }

  function lineageIntegrity(receipt, visited = new Set()) {
    if (visited.has(receipt.review_id)) return "lineage_cycle";
    visited.add(receipt.review_id);

    const receiptAttempt = receipt.attempt || 1;
    if (!receipt.parent_review_id) {
      return receiptAttempt === 1 ? "pass" : "invalid_root_attempt";
    }

    const parentPath = receiptPath(receipt.parent_review_id);
    if (!existsSync(parentPath)) return "parent_unverified";

    let parent;
    try {
      parent = validateReceipt(parseJsonFile(parentPath, "receipt_invalid"), receipt.parent_review_id);
    } catch {
      return "parent_unverified";
    }

    try {
      if (effectiveReceiptProject(parent) !== effectiveReceiptProject(receipt)) return "parent_unverified";
    } catch {
      return "parent_unverified";
    }

    const parentIntegrity = lineageIntegrity(parent, visited);
    if (parentIntegrity === "lineage_cycle") return parentIntegrity;
    if (parentIntegrity !== "pass") return "ancestor_invalid";

    const parentAttempt = parent.attempt || 1;
    if (parentAttempt > MAX_LINEAGE_ATTEMPTS || receiptAttempt > MAX_LINEAGE_ATTEMPTS) {
      return "attempt_policy_exceeded";
    }
    if (receiptAttempt <= parentAttempt) return "attempt_non_monotonic";

    const parentTargets = parent.candidate_files
      .map((file) => resolve(parent.project_path, file.path))
      .sort();
    const receiptTargets = receipt.candidate_files
      .map((file) => resolve(receipt.project_path, file.path))
      .sort();
    if (!sameArray(parentTargets, receiptTargets)) return "scope_mismatch";
  ...
  function receiptReady(receipt, lineageState = lineageIntegrity(receipt)) {
  ...
  function assertReceiptProject(receipt, projectPath) {
    ...
    const candidateRoot = resolveProjectRoot(resolved).projectPath;
    const receiptRoot = effectiveReceiptProject(receipt);
    if (candidateRoot !== receiptRoot && resolved !== receipt.project_path) {
      throw new OrganicRddError("project_mismatch", "Review receipt belongs to a different project workspace.");
    }
  }

  function resolveLineage({ parent_review_id, attempt }, project, candidateFiles) {
    const normalizedAttempt = Number(attempt);
    if (attempt !== undefined && (!Number.isInteger(normalizedAttempt) || normalizedAttempt <= 0)) {
      throw new OrganicRddError("lineage_invalid", "attempt must be a positive integer.");
    }
    if (parent_review_id === undefined || parent_review_id === null) {
      const rootAttempt = attempt === undefined ? 1 : normalizedAttempt;
      if (rootAttempt !== 1) {
        throw new OrganicRddError("lineage_invalid", "A root review must start with attempt 1.");
      }
      return { parent_review_id: null, attempt: rootAttempt };
    }
    ...
    if (effectiveReceiptProject(parent) !== project) {
      throw new OrganicRddError("lineage_invalid", "parent review belongs to a different project.");
    }
    const parentIntegrity = lineageIntegrity(parent);
    if (parentIntegrity !== "pass") {
      throw new OrganicRddError("lineage_invalid", "parent lineage is not healthy.", {
        parent_lineage_integrity: parentIntegrity,
      });
    }
    ...
    const parentTargets = parent.candidate_files
      .map((file) => resolve(parent.project_path, file.path))
      .sort();
    const candidateTargets = candidateFiles
      .map((file) => resolve(project, file.path))
      .sort();
    if (!sameArray(parentTargets, candidateTargets)) {
      throw new OrganicRddError(
        "lineage_scope_mismatch",
        "A successor review must preserve the exact parent candidate files.",
        {
          parent_paths: parent.candidate_files.map((file) => file.path),
          candidate_paths: candidateFiles.map((file) => file.path),
        },
      );
    }

    const parentAttempt = parent.attempt || 1;
    const resolvedAttempt = attempt === undefined ? parentAttempt + 1 : normalizedAttempt;
    if (resolvedAttempt <= parentAttempt) {
      throw new OrganicRddError("lineage_invalid", "A successor attempt must be greater than its parent attempt.");
    }
    if (resolvedAttempt > MAX_LINEAGE_ATTEMPTS) {
      throw new OrganicRddError("lineage_attempt_limit", `A lineage may contain at most ${MAX_LINEAGE_ATTEMPTS} attempts.`);
    }
    return {
  ...
  function start({ project_path, feature_id, files, parent_review_id, attempt }) {
    ...
    const gitRoot = gitState?.root ? normalizePath(gitState.root) : null;
    const warnings = manifestWarnings(candidate.candidate_files, gitState);
    const mode = getMode();
    const timestamp = now();
    const lineage = resolveLineage({ parent_review_id, attempt }, candidate.project_path, candidate.candidate_files);
    ...
    const receipt = {
      ...
      project_id: projectId(candidate.project_path),
    ...
    clearPointer(candidate.project_path);
  ...
  function status(reviewID, projectPath) {
    ...
    const lineageState = lineageIntegrity(receipt);
    const gateReady = currentMode === "managed"
      && receipt.review_mode === "managed"
      && candidateState.candidate_fresh
      && receipt.status === "approved"
      && lineageState === "pass"
      && receiptReady(receipt, lineageState);
    return {
      ...receipt,
      ...candidateState,
      lineage_integrity: lineageState,
      ...
      effective_status: currentMode === "disabled"
        ? "unmanaged"
        : candidateState.candidate_fresh && lineageState === "pass" ? receipt.status : "blocked",
    };
  }
  ...
  function capture({ review_id, lens, status: lensStatus, summary = "", evidence, project_path, findings, reviewer_id, execution_id }) {
    ...
    assertFresh(receipt);
    if (!receipt.required_lenses.includes(lens) && !OPTIONAL_CAPTURE_LENSES.has(lens)) {
  ...
  function finalize(reviewID, projectPath) {
    ...
    const lineageState = lineageIntegrity(receipt);
    if (state.candidate_fresh && receiptReady(receipt, lineageState) && openBlockers.length === 0) {
      receipt.status = "approved";
      receipt.gate_status = "pending";
      delete receipt.block_reasons;
      writePointer(receipt.project_path, receipt.review_id);
    } else {
      receipt.status = "blocked";
      receipt.gate_status = "fail";
      receipt.block_reasons = [
        ...
        ...(lineageState !== "pass" ? [`lineage_${lineageState}`] : []),
        ...(openBlockers.length ? [`open_blocker_findings:${openBlockers.length}`] : []),
      ];
      clearPointer(receipt.project_path);
    }
    return saveReceipt(receipt);
  }
  ...
  function gate(reviewID, projectPath, options = {}) {
    ...
    const state = freshness(receipt);
    const currentManifestWarnings = manifestWarnings(
      receipt.candidate_files,
      inspectGit(receipt.project_path, options.stagedOnly ? { stagedOnly: true } : {}),
      receipt.git?.available,
    );
    const manifestStrictFailure = Boolean(options.strict_manifest) && currentManifestWarnings.length > 0;
    const lineageState = lineageIntegrity(receipt);
    const pass = state.candidate_fresh
      && !manifestStrictFailure
      && lineageState === "pass"
      && receipt.status === "approved"
      && receiptReady(receipt, lineageState);
    receipt.gate_status = pass ? "pass" : "fail";
    if (!state.candidate_fresh || lineageState !== "pass") {
      receipt.status = "blocked";
      clearPointer(receipt.project_path);
    }
    saveReceipt(receipt);
    return {
      ...
      reason: !state.candidate_fresh
        ...
        : manifestStrictFailure
          ...
          : lineageState !== "pass"
            ? "lineage_invalid"
            : pass ? "receipt_approved" : "receipt_not_approved",
   ...
   function validate(reviewID, projectPath) {
    const path = receiptPath(reviewID);
    if (!existsSync(path)) {
      return {
        review_id: reviewID,
        receipt_integrity: "not_found",
        candidate_freshness: "blocked",
        lineage_integrity: "blocked",
        policy_compatibility: "unknown",
        project_binding: "blocked",
        git_projection: "unavailable",
        required_action: "start_new_review",
        recommendation: "Receipt not found; start a new review.",
      };
    }

    let receipt;
    try {
      receipt = validateReceipt(parseJsonFile(path, "receipt_invalid"), reviewID);
    } catch (error) {
      return {
        review_id: reviewID,
        ...
        git_projection: "unavailable",
        required_action: "start_new_review",
        recommendation: `Receipt is structurally invalid: ${error instanceof OrganicRddError ? error.message : String(error.message || error)}`,
      };
    }
    ...
    try {
      normalizedProject = projectPath
        ? resolveProjectRoot(projectPath).projectPath
        : effectiveReceiptProject(receipt);
    } catch {
      projectValid = false;
    }

    const state = freshness(receipt);
    const candidateFreshness = state.candidate_fresh ? "pass" : "stale";

    const lineageState = lineageIntegrity(receipt);

    const policyCompatibility = receipt.policy_snapshot?.classifier_version
      ? (receipt.policy_snapshot.classifier_version === CLASSIFIER_VERSION ? "pass" : "outdated")
      : "unknown";

    let projectBinding;
    if (!receipt.project_id) {
      projectBinding = "legacy";
    } else {
      const sameProject = projectValid && normalizedProject === effectiveReceiptProject(receipt);
      projectBinding = sameProject ? "pass" : "blocked";
    }

    const gitState = !receipt.git?.available || receipt.git.error
      ? "unavailable"
      : (receipt.manifest_warnings?.length ? "incomplete" : "complete");

    let requiredAction = "none";
    let recommendation = "Receipt is valid; no action required.";
    if (projectBinding === "blocked") {
      requiredAction = "start_new_review";
      recommendation = "Receipt belongs to a different project; start a review in the correct project workspace.";
    } else if (candidateFreshness === "stale") {
      requiredAction = "start_successor";
      recommendation = "Candidate bytes changed; start a new review with parent_review_id linkage.";
    } else if (lineageState !== "pass") {
      requiredAction = "resolve_lineage";
      recommendation = `Lineage failed the current scope and attempt policy: ${lineageState}.`;
    } else if (receipt.status === "blocked" || receipt.block_reasons?.length) {
      requiredAction = "start_successor";
      recommendation = "Receipt is blocked; start a successor review.";
    }
