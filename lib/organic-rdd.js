...
export function createOrganicRdd({ storeDir } = {}) {
  ...
  function gate(reviewID, projectPath, options = {}) {
   ...
   function validate(reviewID, projectPath) {
    ...
    let lineageIntegrity = "pass";
    if (receipt.parent_review_id) {
      const parentPath = receiptPath(receipt.parent_review_id);
      if (!existsSync(parentPath)) {
        lineageIntegrity = "parent_unverified";
      } else {
        try {
          const parent = validateReceipt(parseJsonFile(parentPath, "receipt_invalid"), receipt.parent_review_id);
          if (effectiveReceiptProject(parent) !== effectiveReceiptProject(receipt)) lineageIntegrity = "parent_unverified";
        } catch {
          lineageIntegrity = "parent_unverified";
        }
      }
    ...
    } else if (lineageIntegrity === "parent_unverified") {
      requiredAction = "resolve_lineage";
      recommendation = "Parent receipt is missing or moved; lineage chain is broken.";
