...
function classifyOne(inputPath) {
  ...
  const runtimeCode = (!/^tests?\//.test(lower) && /\.(?:[cm]?js|ts)$/.test(lower))
  ...
  if (
    ...
    || /\.(?:sh|toml|ya?ml)$/.test(lower)
  ) {
    return { tier: 2, reasons: ["central OpenCode configuration"] };
  }

  if (/^(skills|commands|prompts|injects|spec)\//.test(lower)) {
...
function inspectCandidate(projectPath, files) {
  ...
  for (const input of files) {
    ...
    let fd;
    try {
      fd = openSync(actualPath, constants.O_RDONLY | (constants.O_NOFOLLOW || 0));
      ...
      const projectRelative = normalizePath(relative(project, openedPath));
...
function validateReceipt(receipt, reviewID) {
  ...
  if (receipt.status === "approved") {
    ...
    if (receipt.review_mode !== "managed" || !allLensesPass || !verified) invalid("inconsistent approval");
  }
  return receipt;
}
...
export function createOrganicRdd({ storeDir } = {}) {
  ...
  function start({ project_path, feature_id, files }) {
    ...
    const receipt = {
      ...
      revision: 0,
      review_id: `r_${Date.now()}_${randomUUID()}`,
  ...
  function capture({ review_id, lens, status: lensStatus, summary = "", evidence, project_path }) {
    ...
    receipt.status = failedLenses(receipt).length > 0 || new Set(["fail", "blocked"]).has(receipt.verify_status)
      ? "blocked"
  ...
  function finalize(reviewID, projectPath) {
    ...
    if (state.candidate_fresh && receiptReady(receipt)) {
      receipt.status = "approved";
      receipt.gate_status = "pending";
